...
class Binaria extends CI_Controller
{
	...
	public function index()
	{
		if ($this->session->userdata('is_logged_in')) {
			...
			if ($this->session->userdata('ROL') == 'Ultra' || $this->session->userdata('ROL') == 'SocioAdmin' || $this->session->userdata('ROL') == 'Socio') {
				...
				$result['ganancia'] = $this->model_wallet->gananciasHoy();
				$result['perdida'] = $this->model_wallet->perdidasHoy();
				$result['retiro'] = $this->model_wallet->retiro();
				$result['deposito'] = $this->model_wallet->deposito();
				$fecha = $this->model_proceso->traerInversion();
				if (count($fecha) == 1) {
					if ($this->session->userdata('ID') == 11) {
	...
	public function comprarBinaria($token)
	{
		...
		$empresa = $this->model_proceso->consultar_referido_niveles(2);
			...
			if (count($papa) == 1) {
				...
				if (count($abuelo) == 1) {
					...
					if (count($bisabuelo) == 1) {
						...
						//pago empresa
						$billetera_empresa = $this->model_proceso->cargar_billetera($empresa->token);
						$data4 = array(
							"cuenta_compra" => $billetera_empresa->cuenta_compra + $servicio->precio - $resultado - $resultado2 - $resultado3,
						);
						$this->model_proceso->actualizar_wallet($data4, $billetera_empresa->token);
						...
						$billetera_empresa = $this->model_proceso->cargar_billetera($empresa->token);
						$data1 = array(
							"cuenta_compra" => $billetera_empresa->cuenta_compra + $servicio->precio - $resultado - $resultado2,
						);
						$this->model_proceso->actualizar_wallet($data1, $billetera_empresa->token);
					...
					$billetera_empresa = $this->model_proceso->cargar_billetera($empresa->token);
					$data1 = array(
						"cuenta_compra" => $billetera_empresa->cuenta_compra + $servicio->precio - $resultado,
					);
					$this->model_proceso->actualizar_wallet($data1, $billetera_empresa->token);
					//Activacion del servicio
					$fecha_actual = date("Y-m-d");
				...
				$billetera_empresa = $this->model_proceso->cargar_billetera($empresa->token);
				$data1 = array(
					"cuenta_compra" => $billetera_empresa->cuenta_compra + $servicio->precio,
				);
				$this->model_proceso->actualizar_wallet($data1, $billetera_empresa->token);
				//Activacion del servicio
				$fecha_actual = date("Y-m-d");
