                        ...
                        <div class="col-md-12">
                            <form action="<?= base_url() ?>Puzzle/comprarPuzzle/<?= $perfil->token ?>" method="post">
                                ...
                                <center>
                                    ...
                                    <div class="container">
                                        <select aria-label="Default select example" class="form-control" id="tipo"
                                            name="tipo" required>
                                    ...
                                    <p style="color: green;">Habilitado para ganar comision</p>
                                    <?php } else { ?>
                                    <p style="color: red;">Inhabilitado para ganar comision</p>
                                    <?php } ?>
                                    <p class="text-muted">Recuerda que para iniciar a comisionar, debes de haber
                                        comprado como minimo un rompecabeza</p>
                                    <p>Puede promocionar el rompecabeza en el siguiente enlace: <a
                                            href="<?=base_url()?>Rompecabeza/<?=$perfil->id?>"><?=base_url()?>/Rompecabeza/<?=$perfil->id?></a>
                                    </p>
                                </center>

                                <div class="modal fade" id="compra" tabindex="-1" aria-labelledby="exampleModalLabel"
                                    aria-hidden="true">
                                            ...
                                            <div class="modal-body">
                                                <div class="container">
                                                    <select aria-label="Default select example" class="form-control"
                                                        id="domicilio" name="domicilio" required>
        ...
        $.ajax({
            ...
            success: function(resp) {
                ...
                if (id == 1) {
                    html = '';
                    html +=
                        '<select aria-label="Default select example" class="form-control" name="rompecabeza" required>';
                    html += '<option selected>Seleccionar Rompecabeza</option>';
                    $.each(resp, function(key, value) {
                        html += '<option value="' + value.id + '">' + value.nombre +
                            '-' + value.fichas + ' fichas, $' + Number(parseFloat(
                                    value.valor) + parseFloat(value.costo) +
                                parseFloat(value.envio)) + '</option>';
                    })
                    html += '</select>'
                    $('#add').html(html);
        ...
        $.ajax({
            ...
            success: function(resp) {
                ...
                if (id == 2) {
                    html = '';
                    html +=
                        '<select aria-label="Default select example" class="form-control" name="municipio" required>';
                    ...
                    html +=
                        '<input class="form-control" type="text" name="nota" placeholder="observacion">';
