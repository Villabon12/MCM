...
class Model_operativa extends CI_Model
{
	//metodo contructor 
	...
	function traerDataBinarias()
	{
		$traer = "SELECT * from operativa_envivo where servicio=1";
		$query = $this->db->query($traer);
		return $query->result();
	}
	function traerDataForex()
	{
		$traer = "SELECT * from operativa_envivo where servicio=2";
		$query = $this->db->query($traer);
		return $query->result();
	}
	public function updateLink($datos, $id)
	{
		$this->db->where('id', $id);
		$this->db->update('operativa_envivo', $datos);
	}
